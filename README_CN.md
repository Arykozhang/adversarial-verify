...
你深入某个领域——代码、策略、研究、产品——明知有盲区但自己看不到。这个 skill 让你用结构化方式获得外部对抗视角。

| 角色 | 怎么用 |
|---|---|
| **工程师** | 打包系统架构 + 源码 → 找到正则漏洞、逻辑缺口、测试覆盖不到的场景 |
| **创业者 / 策略人** | 打包商业计划 + 市场分析 → 在假设烧钱之前，先被对抗AI压力测试 |
| **产品经理** | 打包 PRD + 竞品拆解 → 找到 UX 矛盾、遗漏的边缘场景 |
| **研究员** | 打包方法论 + 数据集 → 审计论证链中的跳跃和未言明的假设 |
| **安全工程师** | 打包系统设计 + 威胁模型 → 让对抗AI猎取你已经看习惯的攻击面 |
| **投资者 / 分析师** | 打包投资逻辑 → 找一个真正读了材料的魔鬼代言人 |

共同点：**你有一个感觉已经做到八成的系统或论证，需要外部力量告诉你剩下两成藏在哪里。**
